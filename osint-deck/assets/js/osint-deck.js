...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderDeckElement(t) {
    ...
    const layers = Math.min(orderedCards.length, 3); // stack de hasta 3
    ...
    for (let i = 0; i < layers; i++) {
      ...
      card.classList.add(`layer-${i}`);
      card.style.zIndex = String(10 + (layers - i));
      ...
      // Web / Open Source / Libre logic for chips
      // Assuming these come from t.info.tipo, t.info.licencia, t.info.acceso mapped to chips
  ...
  function renderActions() {
    return `
      <div class="osint-actions">
        <button class="osint-btn-animated osint-act-go">
  ...
  function attachActionEvents(card, urlTemplate, tool, detection) {
    ...
    if (goBtn) {
      ...
      goBtn.classList.toggle("is-disabled", disableActions);
